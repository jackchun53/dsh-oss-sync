 ...
 * `settings.yaml`.
 */
export const LEGACY_ENTRY_ALIASES: Readonly<Record<string, string>> = {
 ...
 */
export function withoutExpressions(value: unknown): unknown {
  if (isExpression(value)) return undefined
  if (Array.isArray(value)) return value.map(item => withoutExpressions(item) ?? null)
 ...
 */
export function expressionPaths(value: unknown, path: string[] = []): string[][] {
  if (isExpression(value)) return [path]
 ...
 */
export function sameSection(left: Record<string, unknown> | undefined, right: Record<string, unknown> | undefined): boolean {
 ...
 */
export function aliasDocument(document: Record<string, unknown>): SettingsDocument {
  const view: SettingsDocument = {}
  for (const [key, section] of Object.entries(document)) {
    if (key === LEGACY_SYNC_NAMESPACE || !isMapping(section)) continue
    const alias = LEGACY_ENTRY_ALIASES[key]
    if (alias !== undefined) {
      if (!Object.hasOwn(document, alias)) view[alias] = section
      continue
    }
    view[key] = section
  }
...
export function planSync(
  local: SettingsDocument, remote: SettingsDocument, baseline: SyncBaseline | undefined, force?: 'push',
): SyncPlan {
  const plan: SyncPlan = { apply: {}, upload: {} }
  for (const [entry, section] of Object.entries(local)) {
    ...
    if (!known) {
      if (stored !== undefined) {
        if (!sameSection(section, stored)) plan.apply[entry] = stored
    ...
    if (localChanged) {
      if (!sameSection(section, stored)) plan.upload[entry] = section
...
export function isSynced(
  entry: string, include: readonly string[], exclude: readonly string[], own: readonly string[],
): boolean {
  if (own.includes(entry)) return false
  if (include.length > 0) return include.includes(entry)
