...
import { CredentialsConfigSchema, mergeConnection, readConfig, resolveConfig, sameConnection, } from './config.js';
...
function asDocument(value) {
    if (!isDocument(value))
        throw new Error('dsh-oss-sync: stored credentials must carry "refs" and "records" maps');
...
export class OssCredentialProvider extends CredentialProvider {
    static Config = CredentialsConfigSchema;
    /**
    ...
    resolve(ref) {
        ...
        if (ambient !== undefined && ambient.length > 0)
            return Promise.resolve({ value: ambient, source: ENV_SOURCE });
    ...
    async set(ref, value) {
        if (value.length === 0)
            throw new Error(`dsh-oss-sync: refusing to store an empty value for "${ref}" (use unset)`);
    ...
    async unset(ref) {
        this.refuseShadowed(ref);
        if (this.local.refs[ref] === undefined)
            return;
    ...
    listRecords() {
        return Promise.resolve(Object.entries(this.local.records)
            .map(([key, record]) => ({ key: key, kind: record.kind })));
    }
    /**
    ...
    async deleteRecord(key) {
        if (this.local.records[key] === undefined)
            return;
        await this.enqueue(() => this.commit((document) => {
    ...
    desiredSpec() {
        ...
        if (reported === undefined)
            return mergeConnection(this.bootstrap, this.connection);
    ...
    async commit(edit) {
        if (!this.store.configured)
            return this.commitLocally(edit);
    ...
    refresh() {
        return this.enqueue(async () => {
            ...
            for (const ref of new Set([...Object.keys(previous.refs), ...Object.keys(next.refs)])) {
                if (previous.refs[ref] !== next.refs[ref])
                    this.notifyUpdated(ref);
            }
