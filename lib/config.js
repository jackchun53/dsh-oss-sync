...
export const OWN_ENTRIES = ['oss-settings', 'oss-credentials'];
...
export function readConfig(config) {
    ...
    for (const [key, value] of Object.entries(config)) {
        ...
        if (current !== undefined)
            plain[key] = Array.isArray(current) ? [...current] : current;
    }
    return plain;
}
/**
 ...
 */
export function clearsConnection(config) {
 ...
 */
export function mergeConnection(base, connection, cleared = false) {
    if (cleared || connection === undefined)
        return base;
    if (base.accessKeyId !== undefined || base.secretAccessKey !== undefined)
        return base;
    return {
        ...base,
 ...
 */
export function locationKey(spec) {
