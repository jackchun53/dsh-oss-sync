...
export class ObjectStore {
     ...
     */
    async preflight() {
        if (!this.configured || this.ambient === undefined)
            return;
        try {
            await this.ambient();
        }
    ...
    async read(key) {
        if (!this.configured)
            throw new Error('dsh-oss-sync: no bucket configured; nothing to read from');
        try {
    ...
    async write(key, text, condition) {
        if (!this.configured)
            throw new Error('dsh-oss-sync: no bucket configured; nothing to write to');
        try {
