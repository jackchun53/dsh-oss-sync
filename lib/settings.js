...
import { DEFAULT_EXCLUDE, OWN_ENTRIES, SyncConfigSchema, clearsConnection, locationKey, mergeConnection, readConfig, resolveConfig, resolveDshHome, sameConnection, } from './config.js';
import { LEGACY_SYNC_NAMESPACE, SYNC_ENTRY, requestVerb, } from './control.js';
import { aliasDocument, expressionPaths, getPath, isMapping, isSynced, planSync, sameSection, setPath, withoutExpressions, } from './document.js';
...
function schemaFields(schema) {
    ...
    if (!isMapping(root) || root['type'] !== 'object' || !isMapping(root['dict']))
        return undefined;
 ...
 */
export class OssSettingsSync extends Service {
    config;
    ...
    startedSignal = () => { };
    whenStarted = new Promise((resolve) => { this.startedSignal = resolve; });
    constructor(ctx, config) {
        ...
        this.poll = new PollLoop(this.spec.pollMs, () => this.sync(), (error) => {
    ...
    async *[Service.init]() {
        ...
        if (clearsConnection(plain))
            await this.forgetLegacyConnection();
    ...
    async start(settings) {
        try {
            const loader = this.lookup('loader');
            await loader?.await?.();
            await this.awaitHarnessImport();
            if (this.closed || this.settings !== settings)
                return;
     ...
     */
    async awaitHarnessImport() {
        const home = this.profile().home ?? resolveDshHome();
        const legacy = join(home, 'settings.yaml');
        if (!existsSync(legacy))
            return;
        for (let waited = 0; waited < 10_000 && existsSync(legacy); waited += 100)
            await delay(100);
    ...
    profile() {
        return this.lookup('profileContext') ?? {};
    }
    /**
     ...
     */
    outsideHmr(write) {
        const executing = this.lookup('hmr')?.executing;
    ...
    async migrate(settings) {
        ...
        if (await this.state.readState(marker) !== undefined)
            return;
        ...
        if (document !== undefined) {
            ...
            for (const [entry, section] of Object.entries(aliasDocument(document))) {
                ...
                if (view === undefined || !this.syncs(entry))
                    continue;
    ...
    desiredSpec(plain = readConfig(this.config)) {
    ...
    async forgetLegacyConnection() {
        if (this.legacyConnection === undefined)
            return;
     ...
     */
    async onVolatileUpdate(paths) {
        if (this.closed)
            return;
        const fields = new Set(paths.map(path => path[0] ?? ''));
        const plain = readConfig(this.config);
        try {
            if ([...fields].some(field => CONNECTION_FIELDS.has(field)))
                await this.reconnect(plain);
            ...
            if (fields.has('include') || fields.has('exclude'))
                this.schedule(0);
     ...
     */
    async apply(settings, entry, section) {
        const redacted = settings.describe({ redactSecrets: true }).find(view => String(view.ns) === entry);
        const full = settings.describe().find(view => String(view.ns) === entry);
        if (redacted === undefined || full === undefined)
            throw new Error(`entry "${entry}" is no longer configurable`);
        const fields = schemaFields(redacted.schema);
        const next = structuredClone(Object.fromEntries(Object.entries(section)
            .filter(([field]) => fields?.has(field) ?? true)));
        for (const secret of redacted.secrets ?? []) {
            const value = getPath(full.user, secret.path);
            if (value !== undefined && secret.path.length > 0)
                setPath(next, secret.path, structuredClone(value));
        }
        for (const path of expressionPaths(full.user)) {
            if (path.length > 0)
                setPath(next, path, structuredClone(getPath(full.user, path)));
        }
    ...
    async loadBaseline() {
        if (!this.baselineLoaded) {
            const stored = await this.state.readState(this.profileState('settings-sync.yaml'));
    ...
    sync(force) {
        return this.enqueue(async () => {
            ...
            if (this.closed || !this.started || settings === undefined)
                return;
            ...
            for (let attempt = 1; attempt <= MAX_WRITE_ATTEMPTS; attempt += 1) {
                ...
                const envelope = remote === undefined ? undefined : parseEnvelope(remote.text);
                ...
                for (const [entry, section] of upload)
    ...
    publishStatus() {
        ...
        const reference = this.config.status;
        ...
        if (this.settings === undefined)
            return;
        try {
            this.owner.emit('settings/document-updated', SYNC_ENTRY, 0);
        }
        catch (error) {
            this.owner.logger.debug(error);
        }
